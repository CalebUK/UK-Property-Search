...
// ... (Rest of logic: startSearch, loadProperty, etc. same as before) ...

window.startSearch = async function() {
    const input = document.getElementById('postcode-input');
    const status = document.getElementById('status-msg');
    if(input.value.length < 3) { status.innerText = "Enter location"; return; }
    ...
    // 4. Generate Houses (Mock for now)
...
// Helpers needed for startSearch
async function getCoordinates(postcode) {
    ...
    // Try Nominatim for Town Names
    try {
        const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(postcode)}, UK`);
        const d = await r.json();
        if(d && d.length > 0) return { lat: parseFloat(d[0].lat), lon: parseFloat(d[0].lon), area: postcode };
    } catch { return null; }
    return null;
}
...
function getStationConnections(name) {
    for(const [k,v] of Object.entries(STATION_CONNECTIONS)) if(name.includes(k)) return v;
    return null;
}
...
window.toggleSettings = function() {
    ...
    else { modal.style.display = 'flex'; document.getElementById('api-key-input').value = apiKey; }
}
